...
class PheromoneShaderManager {
    ...
    fun create() {
        val vert = Gdx.files.internal("shaders/pheromone/pheromone.vert").readString()
    ...
    fun dispose() {
        ...
        mesh.dispose()
        if (ssbo[0] != 0) {
            val buf = BufferUtils.newIntBuffer(1).apply { put(ssbo[0]); flip() }
